---
title: "Word Cloud-Generator für die Veröffentlichungsdatenbanken <br>bei IT.NRW"
author: "Lars Hofmann"
date: "`r Sys.Date()`"
output: 
...
```{marginfigure}
Die SVG-Dateigröße von `svglite` schrumpft im Vergleich zur Standard-SVG-Schnittstelle von R auf ein Fünfundzwanzigstel (ca. 17 kb)!
```

Zusammen mit dem R-Paket [`svglite`](https://cran.r-project.org/web/packages/svglite/) können diese Schlagwortwolken als extrem kleine SVG-Dateien bereitgestellt werden. Die Texte bleiben erhalten (keine Pfadumwandlung) und können komplett nachbearbeitet werden.
...
```{marginfigure}
Die Vorgehensweise ist abhängig vom Betriebssystem.
```
